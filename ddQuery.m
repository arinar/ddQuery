...
%                     repr(end+1, :) = {char(10)}; disp([repr{:}])
	...
	methods(Access=private, Static)
			...
			for act = [',' combinators; selectors]
				...
				if regexp(act{2}, '^\$\d+$', 'match', 'once') % selector is a reference ~> simple column-number
					...
					if ~isempty(selector.kind)
						common_find_args = [common_find_args 'ObjectKind', selector.kind]; %#ok<AGROW>
					end
					...
					for attr = selector.attributes % parameters
						if ~isempty(attr.ref) % the value is a ref-property ~> append to find_args later
							...
							[attr.name, attr.value] = ddQuery.wrap_find_arg(attr.name, attr.operator, attr.value);
							common_find_args = [common_find_args, attr.name, attr.value]; %#ok<AGROW>
						end                        
					end                    
				...
				for i = 1:size(infos, 1)
					...
					for j = 1:numel(rattrs)
						if strcmp(rattrs(j).value, 'Name')
							value = dsdd('GetAttribute', infos(i, 1 + j), 'Name');
						elseif ismember(rattrs(j).value, {'Kind', 'ObjectKind'})
							value = dsdd('GetAttribute', infos(i, 1 + j), 'ObjectKind');
						else
							value = dsdd('Get', infos(i, 1 + j), rattrs(j).value);
						end
						[name, value] = ddQuery.wrap_find_arg(rattrs(j).name, rattrs(j).operator, value);
						find_args = [find_args, name, value]; %#ok<AGROW>
					end
		...
		function [name, value] = wrap_find_arg(name, operator, value)
			switch operator
			...
			switch lower(name)
				case 'name', name = 'Name';
				case 'objectkind', name = '$ObjectKindRegexp'; %NOTE: this is not a property but a magic name for ddQuery.find
				case 'handle', error('cannot handle handle');
			end
		end    
	end
